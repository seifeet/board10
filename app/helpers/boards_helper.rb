module BoardsHelper
  def correct_board
    ...
    redirect_to(@board) unless @board.member?(current_user) || (current_user && current_user.admin?)
  ...
  def correct_owner
    @board = Board.find(params[:id])
    redirect_to(@board) unless current_user.owner?(@board) || (current_user && current_user.admin?)
  rescue ActiveRecord::RecordNotFound
    page_not_found
  end
